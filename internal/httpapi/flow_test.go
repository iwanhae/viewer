...
func TestFinalizeStatusReportsAlbumEmbeddingProgress(t *testing.T) {
	harness := newFlowHarness(t)

	imageA := testPNG(t, 4, 2, 11)
	imageB := testPNG(t, 2, 6, 22)
	zipData := testZip(t,
		map[string][]byte{"001.png": imageA, "002.png": imageB},
		[]string{"001.png", "002.png"},
	)

	albumID := harness.uploadZip(t, "progress.zip", zipData)
	harness.finalizeAndWait(t, albumID)

	rec := harness.do(t, http.MethodGet, "/api/albums/"+albumID+"/finalize", nil)
	if rec.Code != http.StatusOK {
		t.Fatalf("finalize status=%d body=%s", rec.Code, rec.Body.String())
	}
	var state struct {
	...
	if err := json.Unmarshal(rec.Body.Bytes(), &state); err != nil {
		t.Fatalf("decode finalize state: %v body=%s", err, rec.Body.String())
	}
	if state.Status != "SUCCEEDED" {
		t.Fatalf("status=%q want SUCCEEDED", state.Status)
	}
	// The harness wires a provider but never starts the embedding workers, so
	// the album is indexed with both blobs still pending. That is exactly the
	// state the upload page has to be able to show.
	if state.Embedding == nil {
		t.Fatalf("expected an embedding payload, got: %s", rec.Body.String())
	}
	if !state.Embedding.Enabled {
		t.Fatalf("expected Enabled=true with a provider wired: %+v", state.Embedding)
	}
	if state.Embedding.Total != 2 || state.Embedding.Pending != 2 || state.Embedding.Ready != 0 {
		t.Fatalf("embedding=%+v want total=2 pending=2 ready=0", state.Embedding)
	}

	// The global view agrees with the album view.
	globalRec := harness.do(t, http.MethodGet, "/api/embedding", nil)
	if globalRec.Code != http.StatusOK {
		t.Fatalf("embedding status=%d body=%s", globalRec.Code, globalRec.Body.String())
	}
	var global struct {
		Total   int `json:"total"`
		Pending int `json:"pending"`
	...
	if err := json.Unmarshal(globalRec.Body.Bytes(), &global); err != nil {
		t.Fatalf("decode global embedding status: %v body=%s", err, globalRec.Body.String())
	}
	if global.Total != 2 || global.Pending != 2 {
		t.Fatalf("global embedding=%+v want total=2 pending=2", global)
	}
}
