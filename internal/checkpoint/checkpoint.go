...
func ensureFile(ctx context.Context, dir, base, name string) error {
	...
	log.Printf(
		...
		url, progress.FormatSize(float64(size)), elapsed.Round(time.Millisecond), progress.FormatSize(float64(size)/elapsed.Seconds()),
	)
	return nil
}
...
// copy runs, the progress package logs position and rate on its interval.
func download(ctx context.Context, url, tmp, dst string) (int64, error) {
	...
	body := progress.NewReader(resp.Body)
	stopProgress := progress.Report("checkpoint", "fetching", filepath.Base(url), body.Count, resp.ContentLength)
